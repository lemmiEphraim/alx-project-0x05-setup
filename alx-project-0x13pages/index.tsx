...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      setCurrentPrompt(responseData?.prompt || "");
    }
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-4">
      <div className="flex flex-col items-center">
        ...
        {imageUrl && <ImageCard action={() => setImageUrl(imageUrl)} imageUrl={imageUrl} prompt={currentPrompt} />}
      </div>
      {
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 border max-w-full md:max-w-[1100px] p-2 overflow-y-scroll h-96">
              {generatedImages?.map(
                ({ imageUrl, prompt }: ImageProps, index) => (
                  <ImageCard
                    action={() => { setImageUrl(imageUrl); setCurrentPrompt(prompt); }}
                    imageUrl={imageUrl}
                    prompt={prompt}
                    key={index}
                    width="w-full"
